...
use iced_fonts::{
    bootstrap::{caret_left_fill, caret_right_fill, check, x},
    BOOTSTRAP_FONT,
};
    ...
    'b: 'a,
{
        ...
        let month_year = Row::<Message, Theme, Renderer>::new()
            ...
            .push(
                Row::new()
                    ...
                    .push(
                        // Left Month arrow
                        Container::new(caret_left_fill().size(font_size.0 + 1.0))
                            .height(Length::Shrink)
                            .width(Length::Shrink),
                    )
                    .push(
                    ...
                    .push(
                        // Right Month arrow
                        Container::new(caret_right_fill().size(font_size.0 + 1.0))
                            .height(Length::Shrink)
                            .width(Length::Shrink),
                    ),
            )
    ...
    fn update(
        ...
        self.update_month_year(&event, month_year_layout, cursor, shell);
        ...
        self.update_days(&event, days_layout, cursor, shell);
...
fn month_year(
    ...
    let mut f = |layout: Layout<'_>, text: &str, target: Focus, font_size: Pixels| {
        ...
        renderer.fill_text(
            iced::advanced::Text {
                content: char::from_u32(0xF22D).unwrap().to_string(),
        ...
        renderer.fill_text(
            iced::advanced::Text {
                content: char::from_u32(0xF231).unwrap().to_string(),
